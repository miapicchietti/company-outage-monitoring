...
- **`is_outage()`** counts a check as an outage for: any connection-level error (timeout, DNS failure, refused/reset — not currently split apart, see below), no status code at all, any 5xx, or a 404. Everything else (2xx, 3xx, and non-404 4xx like 403/429) is treated as healthy — 403/429 specifically mean "site is up but blocking automated traffic," not down.
- **Known incomplete work**: `is_outage()`'s connection-error handling is broader than it should be. Real data showed ~85% of generic "Connection Error" cases are transient/false-positive (mostly anti-bot reactions to scanning traffic), while DNS-resolution failures specifically are reliably permanent. The plan (discussed, never implemented) is to split these apart — keep DNS failure as a real outage signal, drop refused/reset.
- **The 30-second recheck** (`if is_outage(result): sleep 30; recheck`) exists because a single failed check is sometimes just transient flakiness (confirmed directly on a Korean-hosted vendor that failed ~50% of the time regardless of any code changes — genuine international connection instability, not a bug). This doubles worst-case time-per-company on a failure, which is a real cost at ~1,800 companies.
