...
- **The 30-second same-client recheck was removed.** It used to retry a failed check once (`sleep 30; recheck`) to absorb transient flakiness. Once `browser_verify()` existed to double-check Timeout/Connection Error with a real browser (a strictly better second opinion — it catches bot-detection blocking the same-client retry never could, not just transient blips), the extra 30s retry became redundant for those cases and was cut to speed up runs. Trade-off: 5xx/404 outages (which `browser_verify()` doesn't cover) now get marked down on a single check with no retry at all.
